...
$(document).ready(function() {
	escuchar_elementos();
	poner_fechas();
	consulta_gastos( $("#fecha_inicio").val(), $("#fecha_fin").val(),$("#id_usuario").val(),$("#is_admin").val() );
...
function dibuja_tabla_caja(datos){
	...
	for (var i = datos.length - 1; i >= 0; i--) {
		...
		.append( 
			$("<tr>")
			.append(
				$("<td>").html(currency(datos[i].importe)),
				$("<td>").html(datos[i].concepto),
				$("<td>").html(datos[i].descripcion),
				$("<td>").html(datos[i].numero_remision),
...
function consulta_gastos(fecha_inicio, fecha_fin, id,admin){
	$.post('./modulos/gastos/consultar_todos_los_gastos.php', {fecha_inicio: fecha_inicio, fecha_fin: fecha_fin,id:id,admin:admin}, function(respuesta) {
