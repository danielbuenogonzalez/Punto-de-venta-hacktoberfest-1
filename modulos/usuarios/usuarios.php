...
function change_dependencia($id, $dep){
    global $base_de_datos;
  
    $statement = $base_de_datos->prepare("UPDATE usuarios SET dependencia = ? WHERE rowid = ?;");
    
    return $statement->execute([ $dep,$id]);  
}
function comprobar_datos($nombre, $palabra_secreta)
{
    ...
    if (password_verify($palabra_secreta, $palabra_secreta_encriptada)) {
        ...
        $rowid =$fila["rowid"];
        propaga_datos_sesion($nombre, $administrador,$dependencia,$rowid);
        return true;
    } else {
        return false;
    }
...
function insertar_usuario($nombre, $palabra_secreta, $administrador = false,$dependencia=false)
{
    ...
    $sentencia = $base_de_datos->prepare("
		...
		(nombre, palabra_secreta, administrador,dependencia)
		VALUES
		(?,?,?,?)");
    return $sentencia->execute([$nombre, $palabra_secreta_encriptada, $administrador,$dependencia]);
}

function propaga_datos_sesion($nombre_de_usuario, $administrador,$dependencia,$rowid)
{
    global $base_de_datos;
    ...
    $_SESSION["nombre_dependencia"]= @$fila["nombre"];
    $_SESSION["id"] =$rowid;
}
